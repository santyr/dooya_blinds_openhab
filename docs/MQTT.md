...
Use examples/openhab.things and examples/openhab.items after replacing IDs and broker. Disable autoupdate on the Rollershutter Item; treat stale UNDEF and OFFLINE explicitly. The example Thing also watches service/availability so the MQTT last will marks it offline if the daemon crashes. Check the separate per-shade availability Item before automation. Verify UNDEF propagation and Thing-offline behavior with the installed openHAB version. Generic MQTT scalar topics avoid requiring JSONPath for basic control. JSON telemetry can be linked later after validation.
